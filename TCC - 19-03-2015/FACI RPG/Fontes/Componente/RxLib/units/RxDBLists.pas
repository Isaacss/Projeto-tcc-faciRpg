...
uses SysUtils, Classes, DB, DBTables, RxDBUtils, RxBdeUtils,
{$IFNDEF VER80} 
  Windows, Bde;
{$ELSE}
...
{$IFDEF RX_D12}
  STableName := AnsiStrAlloc(Length(FTableName) + 1);
{$ELSE}
...
begin
{$IFDEF RX_D12}
  Buffer := StrPCopy(AnsiStrAlloc(Length(AnsiString(DatabaseName)) + 1), AnsiString(DatabaseName));
{$ELSE}
...
begin
{$IFDEF RX_D12}
  Buffer := StrPCopy(AnsiStrAlloc(Length(AnsiString(DriverType)) + 1), AnsiString(DriverType));
{$ELSE}
