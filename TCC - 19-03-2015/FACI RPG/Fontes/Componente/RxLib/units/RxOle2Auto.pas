...
const
  ...
  DISPATCH_METHODPARAMS = DISPATCH_METHOD
    {$IFNDEF VER80} or DISPATCH_PROPERTYGET {$ENDIF};
...
procedure AssignVariant(
  var Dest: {$IFNDEF VER80} TVariantArg; {$ELSE} Variant; {$ENDIF}
  const Value: TVarRec);
begin
{$IFDEF VER80}
...
{$ENDIF}
  if FailedHR(VarToInterface(FObject).GetIDsOfNames(GUID_NULL,
    {$IFNDEF VER80} @P, {$ELSE} P, {$ENDIF} 1, FLocale, @Result)) then
...
{$ENDIF}
  try
    if FailedHR(VarToInterface(FObject).GetIDsOfNames(GUID_NULL,
      {$IFNDEF VER80} @CharBuf, {$ELSE} CharBuf[0], {$ENDIF}
